...
public class SyntaxAnalyzer implements ICompilerStage
{
    ...
    public boolean processStage() {
        ...
        this.tokens = this.compilerClass.getLanguageTools().getLexer().getTokens().stream()
                .filter(token -> token instanceof ArkoiToken)
                .map(token -> (ArkoiToken) token)
                .toArray(ArkoiToken[]::new);
