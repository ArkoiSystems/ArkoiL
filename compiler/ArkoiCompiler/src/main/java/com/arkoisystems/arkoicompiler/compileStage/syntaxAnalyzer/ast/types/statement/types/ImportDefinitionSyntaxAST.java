...
public class ImportDefinitionSyntaxAST extends AbstractStatementSyntaxAST
{
    ...
    public ImportDefinitionSyntaxAST parseAST(final AbstractSyntaxAST parentAST, final SyntaxAnalyzer syntaxAnalyzer) {
        ...
        if (syntaxAnalyzer.matchesNextToken(TokenType.STRING_LITERAL) == null) {
            ...
            this.importFilePathToken = (StringToken) syntaxAnalyzer.currentToken();
        
            if (this.importFilePathToken.getTokenContent().endsWith(".ark"))
                this.importFilePathToken.setTokenContent(this.importFilePathToken.getTokenContent().substring(0, this.importFilePathToken.getTokenContent().length() - 4));
        }
